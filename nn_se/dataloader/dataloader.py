...
def parse_func(record_proto):
  ...
  features = {
      'clean': tf.FixedLenFeature([wav_len], tf.float32),
      'noise': tf.FixedLenFeature([wav_len], tf.float32),
      'mixed': tf.FixedLenFeature([wav_len], tf.float32)
  }
  _, record = tf.parse_single_example(record_proto, features=features)
  return record['clean'], record['noise'], record['mixed']


def get_batch_inputs_from_dataset(datset_name):
  tfrecords_list = Path(PARAM.root_dir).joinpath('datasets', datset_name, "*.tfrecords")
