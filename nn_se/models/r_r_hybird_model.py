...
from .modules import RRHybirdVariables

class RR_HYBIRD_MODEL(Module):
  def __init__(self,
               mode,
               variables: RRHybirdVariables,
               mixed_wav_batch,
               clean_wav_batch=None,
               noise_wav_batch=None):
    super(RR_HYBIRD_MODEL, self).__init__(
        mode,
        variables,
        mixed_wav_batch,
        clean_wav_batch,
  ...
  def forward(self, mixed_wav_batch):
    ...
    p_outputs = self.post_real_networks_forward(r_est_clean_mag_batch, self.mixed_angle_batch, _mixed_wav_len)
    p_est_clean_mag_batch, p_est_clean_spec_batch, p_est_clean_wav_batch = p_outputs
    return (r_est_clean_mag_batch, r_est_clean_spec_batch, r_est_clean_wav_batch,
            p_est_clean_mag_batch, p_est_clean_spec_batch, p_est_clean_wav_batch)

  def get_loss(self, forward_outputs):
    (r_est_clean_mag_batch, r_est_clean_spec_batch, r_est_clean_wav_batch,
     p_est_clean_mag_batch, p_est_clean_spec_batch, p_est_clean_wav_batch) = forward_outputs
    ...
    # region real net losses
    ## frequency domain loss
    self.comp_net_mag_mse = losses.batch_time_fea_real_mse(p_est_clean_mag_batch, self.clean_mag_batch)
    self.comp_net_reMagMse = losses.batch_real_relativeMSE(p_est_clean_mag_batch, self.clean_mag_batch, PARAM.relative_loss_epsilon)
    ...
    self.comp_net_spec_mse = losses.batch_time_fea_complex_mse(p_est_clean_spec_batch, cliped_clean_spec_batch)
    self.comp_net_reSpecMse = losses.batch_complex_relativeMSE(p_est_clean_spec_batch, cliped_clean_spec_batch, PARAM.relative_loss_epsilon)
    self.comp_net_specTCosSimV1 = losses.batch_complexspec_timeaxis_cos_sim_V1(p_est_clean_spec_batch, cliped_clean_spec_batch) # *0.167
    self.comp_net_specFCosSimV1 = losses.batch_complexspec_frequencyaxis_cos_sim_V1(p_est_clean_spec_batch, cliped_clean_spec_batch) # *0.167
    self.comp_net_specTFCosSimV1 = losses.batch_complexspec_TF_cos_sim_V1(p_est_clean_spec_batch, cliped_clean_spec_batch) # *0.167

    ## time domain loss
    self.comp_net_wav_L1 = losses.batch_wav_L1_loss(p_est_clean_wav_batch, self.clean_wav_batch)*10.0
    self.comp_net_wav_L2 = losses.batch_wav_L2_loss(p_est_clean_wav_batch, self.clean_wav_batch)*100.0
    self.comp_net_reWavL2 = losses.batch_wav_relativeMSE(p_est_clean_wav_batch, self.clean_wav_batch, PARAM.relative_loss_epsilon)
    self.comp_net_sdrV1 = losses.batch_sdrV1_loss(p_est_clean_wav_batch, self.clean_wav_batch)
    self.comp_net_sdrV2 = losses.batch_sdrV2_loss(p_est_clean_wav_batch, self.clean_wav_batch)
    self.comp_net_sdrV3 = losses.batch_sdrV3_loss(p_est_clean_wav_batch, self.clean_wav_batch, PARAM.sdrv3_bias) # *0.167
    if PARAM.sdrv3_bias:
      assert PARAM.sdrv3_bias > 0.0, 'sdrv3_bias is constrained larger than zero. _real'
      self.comp_net_sdrV3 *= 1.0 + 60 * PARAM.sdrv3_bias
    self.comp_net_cosSimV1 = losses.batch_cosSimV1_loss(p_est_clean_wav_batch, self.clean_wav_batch) # *0.167
    self.comp_net_cosSimV1WT10 = self.comp_net_cosSimV1*0.167 # loss weighted to 10 level
    self.comp_net_cosSimV2 = losses.batch_cosSimV2_loss(p_est_clean_wav_batch, self.clean_wav_batch) # *0.334
    self.comp_net_stSDRV3 = losses.batch_short_time_sdrV3_loss(p_est_clean_wav_batch, self.clean_wav_batch,
                                                               PARAM.st_frame_length_for_loss,
                                                               PARAM.st_frame_step_for_loss)
    # engregion losses
