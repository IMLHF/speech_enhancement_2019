...
if [ "$site" == "p40" ]; then
  rsync -avh -e "ssh -p 22 -o ProxyCommand='ssh -p 8695 zhangwenbo5@120.92.114.84 -W %h:%p'" --exclude-from='.gitignore' ./* zhangwenbo5@ksai-P40-2:/DATA/disk1/lihongfeng/speech_enhancement_2019_all_exp/$2
elif [ "$site" == "15223" ]; then
  echo "To $user@$site:~/worklhf/speech_enhancement_2019_exp/$2"
  rsync -av -e 'ssh -p '$site --exclude-from='.gitignore' ./* $user@speaker.is99kdf.xyz:~/worklhf/speech_enhancement_2019_exp/$2
fi
